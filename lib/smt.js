...
function replacepeer(settings,str){
	var result=str
	for(var r in settings.replace_peer)
	{
		// peer is object of vars
		var re=settings.replace_peer[r]

		if (r.match("escape")) {
			var level=parseInt(r.match(/escape(\d+)/)[1])
			//tr("!!!!>>>> level :", r , level)
			while(level>0){
				level--
		...
		if( settings.useregex==1){  
			re=new RegExp(re,"g") 
			result=result.replace(r , re)
		}else{
			
			result=result.replaceAll(r , re)
		}	
	}
	return result
}
...
function tru ( obj, dep, show ){
	//for (var i in arguments){
		console.log(util.inspect( obj, { showHidden: show , depth: dep }));
	//}
}
...
var logger = new (winston.Logger)({
    transports: [
      ...
      new (winston.transports.File)({ filename: '../log/serverlog.log' })
    ]
  });
...
exports.revertjson=revertjson
exports.transMonth=transMonth
